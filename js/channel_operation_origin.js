...
var channel_page={
    ...
    init:function (channel_id, full_screen) {
        ...
        this.movies.map(function(movie, index){
            htmlContents+=
                ...
                '>\
                    <span><span class="channel-number">'+movie.num+'</span>\
                    <img class="channel-icon" src="'+movie.stream_icon+'" onerror="this.src=default_movie_icon;"> </span>'+movie.name+
    ...
    changeFavouriteButton:function(current_menu){
        ...
        if(movie!=null){
            if(!LiveModel.favourite_ids.includes(movie.stream_id)){
                // $(action_buttons).text(typeof current_words['add_to_favorite']!='undefined' ? current_words['add_to_favorite'] : 'Add Fav');
                $(action_buttons).text("Add Fav");
                $(action_buttons).data('action','add');
            }else{
                // $(action_buttons).text(typeof current_words['remove_favorites']!='undefined' ? current_words['remove_favorites'] : 'Remove Fav');
    ...
    hoverMenuItem:function (index) {
        ...
        var that=this;
        moveScrollPosition($('#channel-menu-wrapper'),that.menu_items[index],'vertical',false);
