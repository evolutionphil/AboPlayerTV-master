...
var login_page={
    ...
    startApp:function(data){
        ...
        $('#theme-modal-items-container').html(htmlContents);
        var sliderContents='';
        if(adverts.length>0){
        ...
        saveData('background_colors',data.background_colors);
        saveData('epg_text_colors',data.epg_colors);
        ...
        if(settings.background_color)
            assignColorCode(settings.background_color,'background');
        if(settings.focus_color)
            assignColorCode(settings.focus_color,'focus');
        if(settings.epg_text_color)
            assignColorCode(settings.epg_text_color,'epg');

        var htmlContents='',htmlContents1='';
        playlist_urls.map(function(playlist, index){
            htmlContents+=
                '<div class="modal-operation-menu-type-3 playlist-modal-option text-center"\
                ...
                '</div>'
            htmlContents1+=
                '<div class="login-playlist-item-wrapper"\
                    onmouseenter=login_page.hoverPlaylist('+index+')\
                    onclick=login_page.handleMenuClick()\
                >' +
                    playlist.name+
                '</div>';
        })
        $('#playlist-items-container').html(htmlContents);
        $('#login-playlist-items-container').html(htmlContents1);
        that.playlist_doms=$('.login-playlist-item-wrapper');
        home_page.playlist_modal_options=$('.playlist-modal-option');
        if(!data.mac_registered) {
            $('#login-play-list-information').html(
                'Your mac address is not registered yet.<br>\
                -You can activate your mac address and try 7 days free trial in <a class="login-page-link">https://aboplayer.tv</a>\
            ...
            if(data.expire_date<today){
                ...
                if(data.is_trial==1){
                    $('#login-play-list-information').html(
                        'Your trial day is ended now, please try paid plan in <a class="login-page-link">https://aboplayer.tv/activation</a>'
                    ).show();
                }
                else{
                    $('#login-play-list-information').html(
                        'Your account valid duration is ended now, please try extend expire date in <a class="login-page-link">https://aboplayer.tv/activation</a>'
                    ).show();
                }
            }
            else{
                if(data.is_trial==1){  // will show tiral end message
                    $('#login-play-list-information').html(
                        'Your trial day will be ended in '+data.expire_date+', please try paid plan in <a class="login-page-link">https://aboplayer.tv/activation</a>'
                    ).show();
                    that.login();
                }
                    ...
                    if(data.expire_date<date1)
                        $('#login-play-list-information').html(
                            'Your account valid date will be ended in '+data.expire_date+', please try extend expire date in <a class="login-page-link">https://aboplayer.tv/activation</a>'
                        ).show();
                    that.login();
                }
            }
    ...
    fetchPlaylistInformation:function(){
        ...
        $('.mac-address').text(mac_address);
        $('#login-page-mac-address-container').slideDown();
        var keys=this.keys;
    ...
    hoverPlaylist:function(index){
        var keys=this.keys;
        keys.playlist_selection=index;
        $(this.playlist_doms).removeClass('active');
        $(this.playlist_doms[index]).addClass('active');
        moveScrollPosition($('#login-playlist-items-container'),this.playlist_doms[index],'vertical',false);
    },
    ...
    handleMenuClick:function(){
        ...
        if(keys.focused_part==="playlist_selection"){
            if(mac_valid) {
                ...
                showToast("Sorry","You can not use our service now, please activate your device at https://aboplayer.tv/activation and restart your app");
            }
        }
